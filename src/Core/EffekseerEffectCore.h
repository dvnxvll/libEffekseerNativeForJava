...
public:
  ...
  const char16_t *GetTexturePath(int32_t index,
                                 EffekseerTextureType type) const;

  int32_t GetTextureCount(EffekseerTextureType type) const;

#if !defined(SWIGCSHARP)
  bool LoadTexture(char *data, int len, int32_t index,
                   EffekseerTextureType type);
#endif

#if !defined(SWIGJAVA)
  bool LoadTexture(const unsigned char *data, int len, int32_t index,
                   EffekseerTextureType type);
#endif
