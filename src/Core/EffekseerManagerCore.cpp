...
inline void matrixFromValues(::Effekseer::Matrix44 &matrix, float matrixArray0,
                             float matrixArray1, float matrixArray2,
                             float matrixArray3, float matrixArray4,
                             float matrixArray5, float matrixArray6,
                             float matrixArray7, float matrixArray8,
                             float matrixArray9, float matrixArray10,
                             float matrixArray11, float matrixArray12,
                             float matrixArray13, float matrixArray14,
                             float matrixArray15) {
...
inline void matrixFromValues(::Effekseer::Matrix43 &matrix, float matrixArray0,
                             float matrixArray1, float matrixArray2,
                             float matrixArray3, float matrixArray4,
                             float matrixArray5, float matrixArray6,
                             float matrixArray7, float matrixArray8,
...
bool EffekseerManagerCore::Initialize(int32_t spriteMaxCount, bool srgbMode) {
  ...
  if (setting == nullptr) {
    Effekseer::Log(Effekseer::LogType::Error,
                   "Failed to initialize EffekseerManagerCore : GraphicsError");
    return false;
  }

  manager_ = ::Effekseer::Manager::Create(spriteMaxCount);
  renderer_ = ::EffekseerRendererGL::Renderer::Create(
      spriteMaxCount, EffekseerRendererGL::OpenGLDeviceType::OpenGL3);
...
void EffekseerManagerCore::SetBackground(uint32_t glid, bool hasMipmap) {
  if (backgroundtx_ == nullptr) {
    backgroundtx_ = ::EffekseerRendererGL::CreateTexture(
        renderer_->GetGraphicsDevice(), glid, hasMipmap, nullptr);
  }
  auto texture = static_cast<::EffekseerRendererGL::Backend::Texture *>(
      backgroundtx_.Get());
...
void EffekseerManagerCore::SetDepth(uint32_t glid, bool hasMipmap,
                                    int64_t depthFormat) {
  if (depthtx_ == nullptr) {
    depthtx_ = ::EffekseerRendererGL::CreateTexture(
        renderer_->GetGraphicsDevice(), glid, hasMipmap, nullptr);
  } else {
    auto texture =
        static_cast<::EffekseerRendererGL::Backend::Texture *>(depthtx_.Get());
    texture->Init(glid, hasMipmap, nullptr);
  }
...
void EffekseerManagerCore::SetEffectPosition(int handle, float x, float y,
                                             float z) {
  manager_->SetLocation(handle, x, y, z);
}

void EffekseerManagerCore::SetEffectRotation(int handle, float x, float y,
                                             float z) {
  manager_->SetRotation(handle, x, y, z);
}

void EffekseerManagerCore::SetEffectScale(int handle, float x, float y,
                                          float z) {
  manager_->SetScale(handle, x, y, z);
}

void EffekseerManagerCore::SetLayerParameter(int layer, float viewerPosX,
                                             float viewerPosY, float viewerPosZ,
                                             float distanceBias) {
  ::Effekseer::Manager::LayerParameter layerParameter;
  layerParameter.ViewerPosition =
      ::Effekseer::Vector3D(viewerPosX, viewerPosY, viewerPosZ);
...
void EffekseerManagerCore::SetEffectTransformMatrix(
    int handle, float v0, float v1, float v2, float v3, float v4, float v5,
...
void EffekseerManagerCore::SetCameraParameter(float frontX, float frontY,
                                              float frontZ, float posX,
...
void EffekseerManagerCore::SetProjectionMatrix(float v0, float v1, float v2,
  ...
  matrixFromValues(m, v0, v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12,
                   v13, v14, v15);
  renderer_->SetProjectionMatrix(m);
}

void EffekseerManagerCore::SetCameraMatrix(float v0, float v1, float v2,
                                           float v3, float v4, float v5,
                                           float v6, float v7, float v8,
                                           float v9, float v10, float v11,
                                           float v12, float v13, float v14,
...
void EffekseerManagerCore::SetViewProjectionMatrixWithSimpleWindow(
    int32_t windowWidth, int32_t windowHeight) {
  if (manager_ == nullptr) {
    return;
  }

  renderer_->SetProjectionMatrix(::Effekseer::Matrix44().OrthographicRH(
...
void EffekseerManagerCore::SetDynamicInput(int handle, int32_t index,
                                           float value) {
