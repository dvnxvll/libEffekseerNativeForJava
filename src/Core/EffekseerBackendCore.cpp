...
public:
  CustomTextureLoader(Effekseer::Backend::GraphicsDeviceRef graphicsDevice,
                      bool isSrgbMode = false) {
    internalLoader_ = EffekseerRenderer::CreateTextureLoader(
        graphicsDevice, nullptr,
        isSrgbMode ? Effekseer::ColorSpaceType::Linear
...
public:
  Effekseer::TextureRef Load(const EFK_CHAR *path,
                             Effekseer::TextureType textureType) override {
    // Invalid
    return nullptr;
  }

  Effekseer::TextureRef Load(const void *data, int32_t size,
                             Effekseer::TextureType textureType,
                             bool isMipMapEnabled) override {
...
public:
  ...
  Effekseer::MaterialRef Load(const void *data, int32_t size,
                              Effekseer::MaterialFileType fileType) override {
...
Effekseer::RefPtr<EffekseerSettingCore>
    EffekseerSettingCore::effekseerSetting_ = nullptr;

EffekseerSettingCore::EffekseerSettingCore(bool isSrgbMode) {
#ifdef _WIN32
  if (deviceType_ == EffekseerCoreDeviceType::DirectX9) {
    graphicsDevice_ = EffekseerRendererDX9::CreateGraphicsDevice(
        reinterpret_cast<LPDIRECT3DDEVICE9>(userData_[0]));
  } else if (deviceType_ == EffekseerCoreDeviceType::DirectX11) {
    graphicsDevice_ = EffekseerRendererDX11::CreateGraphicsDevice(
        reinterpret_cast<ID3D11Device *>(userData_[0]),
        reinterpret_cast<ID3D11DeviceContext *>(userData_[1]));
  }
#endif
  if (deviceType_ == EffekseerCoreDeviceType::OpenGL) {
    graphicsDevice_ = EffekseerRendererGL::CreateGraphicsDevice(
        EffekseerRendererGL::OpenGLDeviceType::OpenGL3);
  }

  if (graphicsDevice_ != nullptr) {
    SetTextureLoader(Effekseer::MakeRefPtr<CustomTextureLoader>(graphicsDevice_,
                                                                isSrgbMode));
    SetModelLoader(Effekseer::MakeRefPtr<CustomModelLoader>(graphicsDevice_));
    SetMaterialLoader(
        Effekseer::MakeRefPtr<CustomMaterialLoader>(graphicsDevice_));
...
EffekseerSettingCore::GetGraphicsDevice() const {
  return graphicsDevice_;
}

Effekseer::RefPtr<EffekseerSettingCore>
EffekseerSettingCore::create(bool isSrgbMode) {
...
bool EffekseerBackendCore::InitializeWithDirectX11(void *device,
                                                   void *context) {
#ifdef _WIN32
